...
#include "wch/usb/bitbang.h"

//------------------------------------------------------------------------------

#define ENDPOINT0_SIZE  8  // Fixed for USB 1.1, Low Speed.

#define USB_BB_GPIO  EXP(GPIO, USB_BB_PORT)

//------------------------------------------------------------------------------

static usb_bb_t usb_bb_data;
static uint32_t *usb_bb_always0;
...
void usb_bb_setup(void) {
  usb_bb_data.se0_windup = 0;

  // Enable GPIOs, TIMERs
  RCC->APB2PCENR |= EXP(RCC_APB2PERIPH_GPIO, USB_BB_PORT) | RCC_APB2PERIPH_AFIO;

#if USB_BB_DEBUG_TIMING
  ...
  // Enable TIM1
  TIM1->CTLR1 |= TIM_CEN;
#endif  /* USB_BB_DEBUG_TIMING */

  // GPIO Setup
  USB_BB_GPIO->CFGLR = (USB_BB_GPIO->CFGLR & (~(((0xF << (USB_USB_PIN_DP * 4)) | (0xF << (USB_BB_PIN_DM * 4))
#ifdef USB_PIN_DPU
           | (0xF << (USB_BB_PIN_DPU * 4))
#endif
      )))) |
#ifdef USB_BB_PIN_DPU
      (GPIO_SPEED_50MHZ | GPIO_CNF_OUT_PP) << (4 * USB_BB_PIN_DPU) |
#endif
      (GPIO_SPEED_IN | GPIO_CNF_IN_PUPD) << (USB_BB_PIN_DP * 4) |
      (GPIO_SPEED_IN | GPIO_CNF_IN_PUPD) << (USB_BB_PIN_DM * 4);

  // Configure USB_PIN_DM (D-) as an interrupt on falling edge.
  AFIO->EXTICR = EXP(GPIO_PORTSOURCEGPIO, USB_BB_PORT) << (USB_BB_PIN_DM * 2); // Configure EXTI interrupt for USB_PIN_DM
  EXTI->INTENR = 1 << USB_BB_PIN_DM;                                              // Enable EXTI interrupt
  EXTI->FTENR = 1 << USB_BB_PIN_DM;                                               // Enable falling edge trigger for USB_PIN_DM (D-)

#ifdef USB_PIN_DPU
  // This drives USB_BB_PIN_DPU (D- Pull-Up) high, which will tell the host that we are going on-bus.
  USB_BB_GPIO->BSHR = 1 << USB_BB_PIN_DPU;
#endif
...
#endif  /* RV003USB_USE_REBOOT_FEATURE_REPORT */

#if USB_BB_HANDLE_IN_REQUEST
  if (e->custom || endp) {
    // Can reuse data-stack as scratchpad.
    sendnow = __builtin_assume_aligned(data, 4);
    usb_bb_handle_user_in_request(e, sendnow, endp, sendtok, ist);
    return;
  }
#endif  /* USB_BB_HANDLE_IN_REQUEST */
...
// Handle IN (sending data back to PC)
  ...
  if (data0 == e->opaque) {
    usb_bb_send_data(data0, ENDPOINT0_SIZE, 0, sendtok);
    DBG->DMDATA0 = 0;
    DBG->DMDATA1 = 0;
    e->opaque = 0;
    return;
  }
#endif  /* USB_BB_TERMINAL */
  ...
  if (tosend <= 0)
    ...
    usb_bb_send_data(sendnow, tosend, 0, sendtok);
}
...
#if USB_BB_HANDLE_USER_DATA || USB_BB_REBOOT_REPORT || USB_BB_TERMINAL
...
#if USB_BB_HANDLE_USER_DATA
    usb_handle_user_data(e, epno, data_in, length, ist);
#endif

#if USB_BB_HANDLE_TOKEN
    return;
#endif
  } else
#endif  /* USB_BB_HANDLE_USER_DATA || USB_BB_REBOOT_REPORT || USB_BB_TERMINAL */
...
//    usb_bb_log_event(0, s->wRequestTypeLSBRequestMSB, wvi, s->wLength);
...
#if USB_BB_DEBUG
      if (!e->max_len)
        usb_bb_log_event(1234, dl->lIndexValue, dl->length, 0);
#endif  /* USB_BB_DEBUG */
...
#if USB_BB_OTHER_CONTROL
      usb_handle_other_control_message(e, s, ist);
#endif  /* USB_BB_OTHER_CONTROL */
    }
  }
just_ack:
  // Got the right data.  Acknowledge.
  usb_bb_send_data(0, 0, 2, 0xD2); // Send ACK
}

//------------------------------------------------------------------------------

#if !USB_BB_OPTIMIZE_FLASH

void usb_bb_pid_handle_ack(uint32_t dummy, uint8_t *data, uint32_t dummy1, uint32_t dummy2, usb_bb_t *ist) {
