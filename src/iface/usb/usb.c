...
#if IFACE_USB_HANDLE_IN_REQUEST
  if (e->custom || endp) {
...
#endif  /* IFACE_USB_HANDLE_IN_REQUEST */
...
#if IFACE_USB_HANDLE_USER_DATA || IFACE_USB_REBOOT_REPORT || IFACE_USB_TERMINAL
...
#if IFACE_USB_HANDLE_USER_DATA
    usb_handle_user_data(e, epno, data_in, length, ist);
#endif  /* IFACE_USB_HANDLE_USER_DATA */

#if IFACE_USB_HANDLE_TOKEN
    return;
#endif  /* IFACE_USB_HANDLE_TOKEN */
  } else
#endif  /* IFACE_USB_HANDLE_USER_DATA || IFACE_USB_REBOOT_REPORT || IFACE_USB_TERMINAL */
...
#if IFACE_USB_HID_FEATURES
      usb_handle_hid_set_report_start(e, wLength, wvi);
#endif  /* IFACE_USB_HID_FEATURES */
    } else
#if IFACE_USB_HID_FEATURES || IFACE_USB_TERMINAL
    if (reqShl == (0x01a1 >> 1)) {
      // Class read request.
      // The host wants to read back from us. hid_get_feature_report
#if IFACE_USB_HID_FEATURESS
      usb_handle_hid_get_report_start(e, wLength, wvi);
#endif  /* IFACE_USB_HID_FEATURESS */

#if IFACE_USB_USB_TERMINAL
      if ((wvi & 0xFF) == 0xFD) {
...
#if IFACE_USB_OTHER_CONTROL
      usb_handle_other_control_message(e, s, ist);
#endif  /* IFACE_USB_OTHER_CONTROL */
    }
  }
just_ack:
